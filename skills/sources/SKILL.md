...
- **Failed: `site:linkedin.com/groups`.** Returned company pages and nothing else. LinkedIn Groups are effectively invisible to open-web search. Do not plan discovery around them; if the operator wants them, that is a job for `scan` — reading a group the operator has already joined — not a discovery one.
...
**Which doors are open today, stated plainly.** The server registers **fifteen** tools (11 September 2026): `sources`, `sources_add` and `source_decide` for the map; `brief`, `brief_record`, `brief_derive`, `brief_promote`, `brief_confirm` and `brief_readiness` for the market brief; `flag`; `ingest`, which records what a sweep of the **information sources** found as one story — its items, their grouping and the judgement, in a single act; `swept`, which records **that** a source was read and what the page gave, including a read that gave nothing; and the daily loop — `today`, `select` and `posted`. `capture` is specified (`02_functional_architecture.md` §6A) and was **not built**: `ingest` is the door that closed that gap, and it is a different shape — the unit of the write is the story, not the item, because a session arrives with the conclusion already formed. The outreach tools (`contacts`, `contacted`) remain specified and unbuilt. **Do not invent a call.** A skill that names a tool the server does not expose is worse than one that says the gap out loud — the operator finds out at the point of failure instead of at the point of planning. `mcp/kaveno_mcp/server.py` is what says which exist today; the fifteen-tool contract in `02` says which are specified, and the two numbers are not the same number.
...
## Record every read, including the barren ones

`swept` records that you read a source: how many items the page offered, how many you
kept, a fingerprint of the page, and the titles you extracted. **Call it for a source
that gave you nothing, too** — that is the case it exists for. A source read and barren
and a source nobody has got to look identical in the map unless you say which happened,
and the quiet-source watch is computed from exactly that difference.

**When the read happened is the server's**; there is no argument for it. And you do not
update a source's freshness yourself: recording the read advances it, forward only.
